...
const url =
  "mongodb+srv://admin:admin@preguntas.shcqana.mongodb.net/?retryWrites=true&w=majority&appName=Preguntas";
...
io.on("connection", (socket) => {
  ...
  socket.on("createGame", async (gameData) => {
    ...
    const selectedQuestions = await questionController.getRandomQuestions(
      gameData.selectedCategories,
      gameData.numberOfQuestions
    );
    ...
    newGame.players.push(gameData.playerName);
    // console.log("PIN generado:", newGame);
  ...
  socket.on("joinGame", ({ playerName, gamePIN }) => {
    ...
    // Emitir el estado completo del juego al jugador que se une
    socket.emit("gameJoined", games[gamePIN]);
  ...
  socket.on("startGame", (pin) => {
    ...
    // Emitir evento gameStarted con todos los datos necesarios
    io.emit("gameStarted", {
      pin: pin,
  ...
  socket.on("endGame", ({ gamePIN, score, playerName }) => {
    games[gamePIN].score.push({ score, playerName });

    // if (games[gamePIN].score.length === 2)
    io.emit("podiumData", games[gamePIN].score);
    // io.to(games[gamePIN]).emit("podiumData", games[gamePIN].score);
  });
