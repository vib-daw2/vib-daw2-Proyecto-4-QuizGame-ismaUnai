...
const jah = "mongodb+srv://alomj257:alomj257@cluster0.eesendj.mongodb.net/quiz";
...
io.on("connection", (socket) => {
  ...
  socket.on("startGame", (pin) => {
    ...
    io.emit("gameStarted", games[pin]);
  });
  socket.on("endGame", (data) => {
    const players = games[data?.pin].players || [];
    console.log("Partida iniciada con PIN:", data);
    const player = players.find((ele) => ele === data?.player);
    if (!games[data?.pin]) {
      return console.log("El juego no existe");
    }
    games[data?.pin].scorePlayer.push({ player, score: data?.score });
    io.emit("podiumData", games[data?.pin]);
  });
...
mongoose.connect(url).then(() => {
  ...
  server.listen(PORT, () => {             //cambiar aqui la ip en clase
