...
const GameView = ({ socket }) => {
  ...
  useEffect(() => {
    const gameDataObj = keyToLowerCase(gameData);
    if (!gameDataObj || !gameDataObj.pin) {
      setError(true);
      return;
    }
    ...
    if (timer > 0) {
      ...
      setTimeout(() => {
        ...
        if (nextIndex < questions.length) {
          ...
          socket.emit("endGame", {
            gamePIN: gameDataObj.pin,
            score,
            playerName,
          });
          navigate(`/podium`, { state: { gameDataObj } });
        }
      }, 3000);
    }
  }, [
    ...
    return (
      <div className="error-container">
        <h3 className="error-message">
          Game data could not be loaded. Please try again.
        </h3>
      </div>
    );
  }
